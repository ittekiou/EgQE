---
layout: math
title: TUP-DN-08｜Tracehoodは単数か、複数か
title_en: TUP-DN-08｜Plural Tracehood ── When Does Something Work as a Trace?
---
# TUP-DN-08｜Plural Tracehood

## When Does Something Work as a Trace?

### Tracehoodは単数か、複数か

> **Something may work as a Trace under one description,  
> and fail to do so under another.**

[TUP-DN-07｜History-bearing は、どこにあるのか｜Where Is History? ── System / Relation / Observer](https://camp-us.net/articles/TUP-DN-07_Where-Is-History_System-Relation-Observer.html)  

---

前稿では、history-bearingを三つの記述へ分けた。

**Appearance**

**Reference**

**Dependency**

そして、

**Appearance ≠ Reference ≠ Dependency**

とした。

では、

Traceはこのどこにあるのか。

一つのsomethingが、

三つすべてでTraceなのか。

一つだけでTraceなのか。

あるいは、

Tracehoodそのものが複数なのか。

---

# 1｜スクリーンショット

Amazonの6,550円を表示したスクリーンショットを考える。

実際にAmazonで6,550円が表示された。

その画面を撮影した。

後日、

価格表示は消えた。

スクリーンショットだけがある。

これはTraceだろうか。

われわれは、

「もちろん」

と言いたくなる。

しかし、

何について「もちろん」なのだろう。

---

# 2｜Semiotic Tracehood

スクリーンショットは、

以前のAmazon画面を指す。

少なくとも、

そう読まれる。

ここでは、

スクリーンショットは、

**Semiotic Trace**

として働くように見える。

しかし、

AP-02で偽造画像を導入した。

実際には6,550円表示が存在しなかった。

AIで画像だけ生成した。

それでも画像は、

「以前のAmazon画面」を指すように読める。

つまり、

semioticにTraceとして働くことと、

そのreferenceがtrueであることは、

同じではない。

**Semiotic Tracehood ≠ Truth.**

---

# 3｜Causal Tracehood

次に、

その画像を見た人が、

「Amazonで6,550円だった」

と信じたとする。

その後、

記事を書く。

誰かに話す。

Amazonについての判断を変える。

この場合、

画像はlater configurationにeffectを持つ。

本物でも、

偽物でも。

ここでは、

画像は、

**Causal Trace**

として働くように見える。

しかし、

注意が必要である。

画像そのものが作用主体なのではない。

画像とのEncounterを含む条件によって、

later configurationが異なる。

だから、

**Causal Tracehood**

という呼び方自体も、

まだ仮称である。

---

# 4｜Phenomenological Tracehood

今度は、

そのスクリーンショットを数か月後に見る。

「ああ、これだ」

と思う。

忘れていた出来事が、

Againとして現れる。

以前とは違う味わいで、

過去が立ち上がる。

ここでは、

画像は、

**Phenomenological Trace**

として働くように見える。

しかし、

画像が存在するだけでは足りない。

誰にもEncounterされなければ、

少なくともこの意味でのTracehoodは現れない。

ここでは、

Tracehoodは、

somethingの属性というより、

**Encounterで立ち上がるconfiguration**

に近い。

---

# 5｜三つは一致しない

ここで三つを並べる。

**Semiotic Tracehood**

prior somethingを指す。

**Causal Tracehood**

later configurationのdifferenceに参与する。

**Phenomenological Tracehood**

later EncounterをAgainとして変える。

これらは、

一致することもある。

しかし、

一致する必要はない。

---

# 6｜False Screenshot

偽造スクリーンショット。

Amazonで6,550円は表示されていなかった。

しかし、

読者は本物だと思う。

すると、

Semioticには、

過去を指すように働く。

しかしreferenceはfalseである。

Causalには、

later judgmentにeffectを持ちうる。

Phenomenologicalには、

後日、

「あの6,550円の件」としてAgainを生成しうる。

つまり、

**false reference**

と

**real effect**

と

**real appearance**

が同時に成立しうる。

ここで、

Tracehoodを一つの真偽値で表すことは難しくなる。

---

# 7｜Unread Screenshot

逆に、

本物のスクリーンショット。

実際の6,550円表示を撮影した。

しかし、

保存されたまま、

誰にも見られない。

この画像について、

Semiotic Tracehoodはあるのか。

Causal Tracehoodはあるのか。

Phenomenological Tracehoodはあるのか。

「ある」と言えば、

potentialをTracehoodに含めることになる。

「ない」と言えば、

EncounterされるまでTraceではないことになる。

ここで、

**actual / potential Tracehood**

という別の区別まで必要になる。

だが、

まだ増やさない。

---

# 8｜Dog

犬が店へ向かう。

翌日も店へ向かう。

ここに、

semiotic referenceは必要だろうか。

犬が昨日の店を、

「昨日の店を指す記号」として扱っている必要はない。

しかし、

prior Encounterとのdependencyはあるかもしれない。

そして、

犬にAgainが現れている可能性もある。

ただし、

そのphenomenologyを人間と同じようには確認できない。

つまりDogでは、

三つのTracehoodを簡単に重ねられない。

---

# 9｜AI

AIでは、

prior textをreferenceできる。

contextによってlater outputも変わる。

したがって、

Semiotic / Causalに相当する記述は可能である。

しかし、

Phenomenological Tracehoodはどうか。

AIに、

**Againが現れる**

と言ってよいのか。

ここは、

QASの問いへ接続する。

しかし、

まだ答えない。

重要なのは、

AIについても、

三つのTracehoodが自動的には一致しない、

ということである。

---

# 10｜Homo sapiens

人間では、

三つが重なりやすい。

写真を見る。

過去を指していると理解する。

その写真によって判断が変わる。

そして、

「あのとき」がAgainとして現れる。

だから、

われわれは三つを一つのTraceとしてまとめてしまいやすい。

しかし、

三つは分離できる。

過去を正しくreferenceしていても、

現在の判断にはeffectがないかもしれない。

effectがあっても、

本人にはAgainとして現れないかもしれない。

Againとして現れても、

そのreferenceがfalseかもしれない。

Homo sapiensこそ、

三つを混同しやすい。

---

# 11｜Tracehoodは属性か

ここで、

**Tracehood**

という語そのものを監査する。

ある画像が、

Tracehoodという属性を持っている。

そう考えると、

また実体化に近づく。

むしろ、

> **X works as a Trace under description D in Encounter E.**

くらいまで弱めたほうがよい。

Xが永続的にTraceなのではない。

どのdescriptionで、

どのEncounterにおいて、

何に対して、

Traceとして働くのか。

ここではTracehoodは、

**contextual / relational / positional**

になる。

---

# 12｜Trace of What?

すると、

最初に必要なのは、

「これはTraceか？」

ではない。

**Trace of what?**

である。

偽造Amazon画像は、

実在した6,550円表示のTraceではない。

しかし、

AI画像生成というOccurrenceのTraceとして扱えるかもしれない。

さらに、

それを読んだ人のlater judgmentとのrelationでは、

別のTrace-positionを取るかもしれない。

つまり、

**Tracehood without “of what?” is underdetermined.**

Traceは、

単独では判定できない。

---

# 13｜Trace for What?

さらに、

**Trace for what?**

もある。

証拠として使うのか。

過去を想起するためか。

later behaviorとのdependencyを調べるためか。

歴史を書くためか。

物語を作るためか。

同じsomethingが、

目的によって異なるTrace-positionを取る。

ただし、

purposeを入れすぎると、

すべてをobserver intentionへ回収する危険がある。

ここも保留する。

---

# 14｜Trace when?

さらに、

**Trace when?**

保存された瞬間か。

later Encounterの瞬間か。

comparisonが行われた瞬間か。

effectが観察された瞬間か。

retrospectiveにhistoryが構成された瞬間か。

Tracehoodに時間を入れると、

「Traceがずっと存在していた」

という想定も疑わしくなる。

Tracehoodそのものが、

**更新されうる。**

---

# 15｜Tracehood can ZURE

同じスクリーンショットが、

ある時点では証拠として扱われる。

後に偽造だと判明する。

すると、

semioticな位置づけが変わる。

しかし、

「騙された」という経験へのeffectは消えない。

さらに年月が経てば、

その画像は、

「AI偽造が問題になった時代」の資料になるかもしれない。

同じsomething。

異なるEncounter。

異なるdescription。

異なるTrace-position。

つまり、

**Tracehood itself can ZURE.**

いや。

より慎重には、

**the description under which X works as a Trace can ZURE.**

---

# 16｜NAZORU

ここでNAZORUも変わって見える。

NAZORUとは、

既存のTraceをそのまま再生することではない。

何かを、

あるTrace-positionでEncounterし、

選び、

読み、

使い、

別のconfigurationへ持ち込むpracticeかもしれない。

しかし、

この定義もまだ早い。

ただ一つ、

**NAZORU ≠ Trace Retrieval**

とは言えそうである。

なぞることは、

保存庫から同じものを取り出すことではない。

---

# 17｜Tracing

Tracingについても同じである。

Tracingは、

すでに完成したTraceを追跡することなのか。

それとも、

somethingをTrace-positionに置くprocessなのか。

もし後者なら、

前稿で出た問い、

**Can there be Trace without Tracing?**

がさらに鋭くなる。

Tracingは、

Traceを発見するだけでなく、

Tracehoodの成立に参与している可能性がある。

しかし、

だからといって、

Traceがすべてobserver constructionだとは限らない。

ここはまだ開いておく。

---

# 18｜Plural Tracehood

ここまでの監査から、

一つの可能性が見える。

Tracehoodは、

単数ではない。

少なくとも、

**Semiotic**

**Causal / Dependency**

**Phenomenological**

という異なる記述がある。

しかも、

三つは独立に動きうる。

だから、

**Trace = X**

という一行定義より、

**under which description does X work as a Trace?**

と問うほうが、

discriminationが増える。

---

# 19｜しかしTraceはまだ必要か

ここで最後の反撃を入れる。

Semioticなら、

sign / referenceでよい。

Causalなら、

effect / dependencyでよい。

Phenomenologicalなら、

Again / appearanceでよい。

ならば、

なぜTraceという語が必要なのか。

三つをまとめるためだけなら、

Traceは便利な総称にすぎない。

それでは、

Translation Gainとして弱い。

Traceが生き残るには、

三つのどれか一つではなく、

**三つがZUREながら同じhistoryをめぐって交差すること**

を区別できる必要があるのかもしれない。

しかし、

「同じhistory」と言った瞬間、

また問題が戻る。

---

# 20｜The Next Question

だから次の問いは、

Tracehoodが単数か複数か、

だけではない。

もっと厄介である。

> **What, if anything, is shared across different Trace-positions?**

SemioticにはA。

CausalにはB。

PhenomenologicalにはC。

それでも、

われわれが全部を、

「同じ6,550円のTrace」

と呼びたくなるのはなぜか。

そこに、

本当に何か共通するものがあるのか。

それとも、

われわれが後から、

同じhistoryとして束ねているだけなのか。

---

> **A Trace may not be a thing.**
> 
> **Tracehood may not be a property.**
> 
> **Tracing may not merely discover what was already there.**

では、

異なるTrace-positionをまたいで、

**何が同じなのか。**

あるいは、

**何も同じでなくてよいのか。**

Trial continues.
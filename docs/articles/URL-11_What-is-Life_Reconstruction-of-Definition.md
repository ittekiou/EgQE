---
layout: math
title: URL-11｜生命とは何か──定義の曖昧さと再構築
title_en: URL-11｜What Is Life? — On the Ambiguity of Definition and Its Reconstruction
---
### **URL-11｜What Is Life?**
## 生命とは何か ──定義の曖昧さと再構築
...
**Life** is a syntactic process in which lag arising from otherness-as-relation is folded inward by the membrane, enters a band of persistence, and continues to be recursively generated.

**Matter** is the mode in which lag arising from otherness-as-relation is not opened to recursion, but is fixed externally as a trace.

**Death** is the transition in which, through the collapse of recursion and ψ-sustained persistence, the life-syntax is reduced to the material mode.

These are not independent definitions, but a continuous formulation derived from a single branching of the trajectory of lag.
...
**物質**とは、他者＝関係から生じるlagが、再帰に開かれず、痕跡として外部に固定される様式である。

**死**とは、再帰とψ持続の崩壊により、生命構文が物質様式へ還元される転換である。

これらは独立定義ではなく、lagの行方の単一分岐から派生する連続定式化である。

---
layout: math
title: "SAW-11｜Observation as a Relation-Update Process: Three Regimes of Lag Processing (S′–O′ Framework)｜v0.9"
---
...
In this work, _lag_ does not denote a temporal delay measured between two events.  
...
$$  
\text{lag} < Z_0 \quad \Rightarrow \quad \text{observationally unresolvable}  
$$

This notion of sub-$Z_0$ lag plays a central role in reinterpreting physical invariances traditionally treated as fundamental principles.
...
- **S′** denotes the updating pole of the relation.
    
- **O′** denotes the updated pole of the relation.
    

Both are defined _relationally_, not ontologically. Neither exists independently of the relation-update process that couples them.
...
This inequality is not an error condition but the **default mode** of relational updating. Even when observational outcomes appear invariant, this only indicates that the generated lag remains below the observational threshold $Z_0$, not that lag is absent.

The S′–O′ framework therefore shifts the locus of explanation:

- from objects to relations,
    
- from geometry to update processes,
    
- from invariance as principle to invariance as **lag-invisible regime**.
